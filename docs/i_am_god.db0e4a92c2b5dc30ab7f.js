/******/ (() => { // webpackBootstrap
let sins = (/* unused pure expression or super */ null && (["Pride", "Greed", "Lust", "Wrath", "Gluttony", "Envy", "Sloth"]));
let displayed_sins = (/* unused pure expression or super */ null && ([])); // Массив для хранения отображённых грехов
let wave_y = 660;
let target_wave_y = 660; // Целевая позиция волны
let collapse = false;
let counter = 0;
let fill_count = 0;
...
function setup() {
  textFont(font);
  createCanvas(displayWidth - displayWidth * 0.02, displayHeight - displayHeight * 0.2);
  backImg.resize(displayWidth, displayHeight);
  wave.resize(displayWidth, 800);
...
function mousePressed() {
  ...
  if (!collapse && counter > 0) {
    target_wave_y -= 130; // Устанавливаем новую цель для позиции волны
  }
  counter++;
}
